...
def expectation_maximization(
    ...
    def beta(x,mu_,sig_,alpha_):
        ...
        diff = x - mu_[None, :]
        #print(x.shape, mu_.shape,L.shape)
        #norm = (L @ diff.T).T
        norm = diff @ L.T
    ...
    for it in range(max_iter):
        ...
        # ws = np.zeros(shape=(K, N))
        # log_probs = normal_dist(X, mus, sigmas)
        # log_pi = np.log(alphas[:, np.newaxis])
                
        # max_log_probs = np.max(log_probs, axis=0) # Maximum log prob of samples
        # tmp = np.log(np.sum( alphas[..., None] * (np.exp(log_probs - max_log_probs))
        ...
        # Nk = ws.sum(1) 
        # mus = (ws[...,None]*X[None,...]).sum(1)/Nk[:,None] 
        # diff = (X[None,...] - mus[:,None,:]) 
        # sigmas = np.einsum('KNi,KNj->Kij',ws[:,:,None]*diff,diff)/Nk[:,None,None]
        # alphas = Nk/N
